...
public class OverworldBiomes {
   ...
   private static Biome biome(Biome.Precipitation pPrecipitation, Biome.BiomeCategory pBiomeCategory, float pTemperature, float pDownfall, MobSpawnSettings.Builder pMobSpawnSettings, BiomeGenerationSettings.Builder pGenerationSettings, @Nullable Music pBackgroundMusic) {
      return biome(pPrecipitation, pBiomeCategory, pTemperature, pDownfall, 4159204, 329011, pMobSpawnSettings, pGenerationSettings, pBackgroundMusic);
   }

   private static Biome biome(Biome.Precipitation pPrecipitation, Biome.BiomeCategory pBiomeCategory, float pTemperature, float pDownfall, int pWaterColor, int pWaterFogColor, MobSpawnSettings.Builder pMobSpawnSettings, BiomeGenerationSettings.Builder pGenerationSettings, @Nullable Music pBackgroundMusic) {
      return (new Biome.BiomeBuilder()).precipitation(pPrecipitation).biomeCategory(pBiomeCategory).temperature(pTemperature).downfall(pDownfall).specialEffects((new BiomeSpecialEffects.Builder()).waterColor(pWaterColor).waterFogColor(pWaterFogColor).fogColor(12638463).skyColor(calculateSkyColor(pTemperature)).ambientMoodSound(AmbientMoodSettings.LEGACY_CAVE_SETTINGS).backgroundMusic(pBackgroundMusic).build()).mobSpawnSettings(pMobSpawnSettings.build()).generationSettings(pGenerationSettings.build()).build();
   }
