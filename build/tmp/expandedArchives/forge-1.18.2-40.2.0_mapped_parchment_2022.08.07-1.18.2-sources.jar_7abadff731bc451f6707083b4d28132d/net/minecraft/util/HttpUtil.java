...
public class HttpUtil {
   ...
   public static String performPost(URL pUrl, Map<String, Object> p_144813_, boolean pLogError, @Nullable Proxy pProxy) {
      return performPost(pUrl, buildQuery(p_144813_), pLogError, pProxy);
   }

   private static String performPost(URL pUrl, String p_144808_, boolean pLogError, @Nullable Proxy pProxy) {
      try {
         if (pProxy == null) {
            pProxy = Proxy.NO_PROXY;
         }

         HttpURLConnection httpurlconnection = (HttpURLConnection)pUrl.openConnection(pProxy);
         ...
         if (!pLogError) {
            LOGGER.error("Could not post to {}", pUrl, exception);
         }
