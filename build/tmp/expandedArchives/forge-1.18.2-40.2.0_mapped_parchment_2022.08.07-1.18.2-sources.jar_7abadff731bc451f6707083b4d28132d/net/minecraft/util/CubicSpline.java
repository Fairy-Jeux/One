...
public interface CubicSpline<C> extends ToFloatFunction<C> {
   ...
   static <C> CubicSpline<C> constant(float pValue) {
      return new CubicSpline.Constant<>(pValue);
   }

   static <C> CubicSpline.Builder<C> builder(ToFloatFunction<C> pCoordinate) {
      return new CubicSpline.Builder<>(pCoordinate);
   }

   static <C> CubicSpline.Builder<C> builder(ToFloatFunction<C> pCoordinate, ToFloatFunction<Float> pValueTransformer) {
   ...
   public static final class Builder<C> {
      ...
      protected Builder(ToFloatFunction<C> pCoordinate) {
         this(pCoordinate, (p_184307_) -> {
            return p_184307_;
         });
      }

      protected Builder(ToFloatFunction<C> pCoordinate, ToFloatFunction<Float> pValueTransformer) {
         this.coordinate = pCoordinate;
         this.valueTransformer = pValueTransformer;
      }

      public CubicSpline.Builder<C> addPoint(float pLocation, float pValue, float pDerivative) {
         return this.addPoint(pLocation, new CubicSpline.Constant<>(this.valueTransformer.apply(pValue)), pDerivative);
      }

      public CubicSpline.Builder<C> addPoint(float pLocation, CubicSpline<C> pValue, float pDerivative) {
