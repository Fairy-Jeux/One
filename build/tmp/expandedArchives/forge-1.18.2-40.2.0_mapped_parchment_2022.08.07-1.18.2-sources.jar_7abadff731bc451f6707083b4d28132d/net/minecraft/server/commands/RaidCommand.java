...
public class RaidCommand {
   ...
   private static int setBadOmenLevel(CommandSourceStack pSource, int pLevel) throws CommandSyntaxException {
      ...
      if (raid != null) {
         ...
         if (pLevel > i) {
            pSource.sendFailure(new TextComponent("Sorry, the max bad omen level you can set is " + i));
         } else {
            int j = raid.getBadOmenLevel();
            raid.setBadOmenLevel(pLevel);
            pSource.sendSuccess(new TextComponent("Changed village's bad omen level from " + j + " to " + pLevel), false);
         }
      } else {
         pSource.sendFailure(new TextComponent("No raid found here"));
      }
   ...
   private static int spawnLeader(CommandSourceStack pSource) {
      pSource.sendSuccess(new TextComponent("Spawned a raid captain"), false);
   ...
   private static int start(CommandSourceStack pSource, int pBadOmenLevel) throws CommandSyntaxException {
      ...
      if (serverplayer.getLevel().isRaided(blockpos)) {
         pSource.sendFailure(new TextComponent("Raid already started close by"));
         return -1;
      } else {
         ...
         if (raid != null) {
            ...
            raids.setDirty();
            pSource.sendSuccess(new TextComponent("Created a raid in your local village"), false);
         } else {
            pSource.sendFailure(new TextComponent("Failed to create a raid in your local village"));
         }
   ...
   private static int stop(CommandSourceStack pSource) throws CommandSyntaxException {
      ...
      if (raid != null) {
         raid.stop();
         pSource.sendSuccess(new TextComponent("Stopped raid"), false);
         return 1;
      } else {
         pSource.sendFailure(new TextComponent("No raid here"));
         return -1;
      }
   }
   ...
   private static int check(CommandSourceStack pSource) throws CommandSyntaxException {
      ...
      if (raid != null) {
         ...
         pSource.sendSuccess(new TextComponent(stringbuilder.toString()), false);
         ...
         pSource.sendFailure(new TextComponent("Found no started raids"));
         return 0;
      }
   }
