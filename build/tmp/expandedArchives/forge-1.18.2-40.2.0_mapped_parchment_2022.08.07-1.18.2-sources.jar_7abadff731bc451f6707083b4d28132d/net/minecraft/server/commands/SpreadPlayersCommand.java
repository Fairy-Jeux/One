...
public class SpreadPlayersCommand {
   ...
   private static int spreadPlayers(CommandSourceStack pSource, Vec2 pCenter, float pSpreadDistance, float pMaxRange, int pMaxHeight, boolean pRespectTeams, Collection<? extends Entity> pTargets) throws CommandSyntaxException {
      ...
      if (pMaxHeight < i) {
         ...
         SpreadPlayersCommand.Position[] aspreadplayerscommand$position = createInitialPositions(random, pRespectTeams ? getNumberOfTeams(pTargets) : pTargets.size(), d0, d1, d2, d3);
         spreadPositions(pCenter, (double)pSpreadDistance, serverlevel, random, d0, d1, d2, d3, pMaxHeight, aspreadplayerscommand$position, pRespectTeams);
         double d4 = setPlayerPositions(pTargets, serverlevel, aspreadplayerscommand$position, pMaxHeight, pRespectTeams);
         pSource.sendSuccess(new TranslatableComponent("commands.spreadplayers.success." + (pRespectTeams ? "teams" : "entities"), aspreadplayerscommand$position.length, pCenter.x, pCenter.y, String.format(Locale.ROOT, "%.2f", d4)), true);
   ...
   private static void spreadPositions(Vec2 pCenter, double pSpreadDistance, ServerLevel pLevel, Random pRandom, double pMinX, double pMinZ, double pMaxX, double pMaxZ, int pMaxHeight, SpreadPlayersCommand.Position[] pPositions, boolean pRespectTeams) throws CommandSyntaxException {
      boolean flag = true;
   ...
   private static double setPlayerPositions(Collection<? extends Entity> pTargets, ServerLevel pLevel, SpreadPlayersCommand.Position[] p_138732_, int pMaxHeight, boolean pRespectTeams) {
      double d0 = 0.0D;
      int i = 0;
   ...
   static class Position {
      ...
      public void randomize(Random pRandom, double pMinX, double pMinZ, double pMaxX, double pMaxZ) {
