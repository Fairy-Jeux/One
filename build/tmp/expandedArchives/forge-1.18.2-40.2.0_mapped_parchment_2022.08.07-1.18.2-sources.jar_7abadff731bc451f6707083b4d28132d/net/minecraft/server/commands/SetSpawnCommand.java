...
public class SetSpawnCommand {
   ...
   private static int setSpawn(CommandSourceStack pSource, Collection<ServerPlayer> pTargets, BlockPos pPos, float pAngle) {
      ...
      if (pTargets.size() == 1) {
         pSource.sendSuccess(new TranslatableComponent("commands.spawnpoint.success.single", pPos.getX(), pPos.getY(), pPos.getZ(), pAngle, s, pTargets.iterator().next().getDisplayName()), true);
      } else {
         pSource.sendSuccess(new TranslatableComponent("commands.spawnpoint.success.multiple", pPos.getX(), pPos.getY(), pPos.getZ(), pAngle, s, pTargets.size()), true);
      }
