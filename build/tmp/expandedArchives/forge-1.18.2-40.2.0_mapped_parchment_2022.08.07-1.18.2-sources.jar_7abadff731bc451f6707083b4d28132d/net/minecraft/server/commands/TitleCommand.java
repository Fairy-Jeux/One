...
public class TitleCommand {
   ...
   private static int showTitle(CommandSourceStack pSource, Collection<ServerPlayer> pTargets, Component pTitle, String p_142784_, Function<Component, Packet<?>> p_142785_) throws CommandSyntaxException {
      ...
      if (pTargets.size() == 1) {
         pSource.sendSuccess(new TranslatableComponent("commands.title.show." + p_142784_ + ".single", pTargets.iterator().next().getDisplayName()), true);
      } else {
         pSource.sendSuccess(new TranslatableComponent("commands.title.show." + p_142784_ + ".multiple", pTargets.size()), true);
      }
