...
public class PerfCommand {
   ...
   private static int startProfilingDedicatedServer(CommandSourceStack pSource) throws CommandSyntaxException {
      ...
      if (minecraftserver.isRecordingMetrics()) {
         ...
         pSource.sendSuccess(new TranslatableComponent("commands.perf.started"), false);
         return 0;
      }
   }
   ...
   private static void saveResults(CommandSourceStack pSource, Path p_180448_, MinecraftServer p_180449_) {
      ...
      try {
         ...
         pSource.sendFailure(new TranslatableComponent("commands.perf.reportFailed"));
      ...
      pSource.sendSuccess(new TranslatableComponent("commands.perf.reportSaved", s1), false);
   }
   ...
   private static void whenStopped(CommandSourceStack pSource, ProfileResults pResults) {
      ...
      pSource.sendSuccess(new TranslatableComponent("commands.perf.stopped", String.format(Locale.ROOT, "%.2f", d0), i, String.format(Locale.ROOT, "%.2f", (double)i / d0)), false);
   }
}