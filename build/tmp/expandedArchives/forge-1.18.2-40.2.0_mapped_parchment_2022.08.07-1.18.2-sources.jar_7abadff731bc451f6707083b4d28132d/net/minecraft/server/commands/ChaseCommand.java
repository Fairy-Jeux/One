...
public class ChaseCommand {
   ...
   private static int stop(CommandSourceStack pSource) {
      if (chaseClient != null) {
         chaseClient.stop();
         pSource.sendSuccess(new TextComponent("You have now stopped chasing"), false);
         chaseClient = null;
      }

      if (chaseServer != null) {
         chaseServer.stop();
         pSource.sendSuccess(new TextComponent("You are no longer being chased"), false);
         chaseServer = null;
      }
   ...
   private static boolean alreadyRunning(CommandSourceStack pSource) {
      if (chaseServer != null) {
         pSource.sendFailure(new TextComponent("Chase server is already running. Stop it using /chase stop"));
         return true;
      } else if (chaseClient != null) {
         pSource.sendFailure(new TextComponent("You are already chasing someone. Stop it using /chase stop"));
         return true;
      } else {
         return false;
      }
   ...
   private static int lead(CommandSourceStack pSource, String pBindAddress, int pPort) {
      if (alreadyRunning(pSource)) {
         ...
         try {
            chaseServer.start();
            pSource.sendSuccess(new TextComponent("Chase server is now running on port " + pPort + ". Clients can follow you using /chase follow <ip> <port>"), false);
         } catch (IOException ioexception) {
            ioexception.printStackTrace();
            pSource.sendFailure(new TextComponent("Failed to start chase server on port " + pPort));
            chaseServer = null;
         }
   ...
   private static int follow(CommandSourceStack pSource, String pHost, int pPort) {
      if (alreadyRunning(pSource)) {
         ...
         chaseClient.start();
         pSource.sendSuccess(new TextComponent("You are now chasing " + pHost + ":" + pPort + ". If that server does '/chase lead' then you will automatically go to the same position. Use '/chase stop' to stop chasing."), false);
         return 0;
      }
   }
}