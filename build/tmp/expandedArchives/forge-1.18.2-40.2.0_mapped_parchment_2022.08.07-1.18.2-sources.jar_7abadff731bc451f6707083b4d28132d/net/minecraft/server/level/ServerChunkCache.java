...
public class ServerChunkCache extends ChunkSource {
   ...
   public ServerChunkCache(ServerLevel pLevel, LevelStorageSource.LevelStorageAccess p_184010_, DataFixer pFixerUpper, StructureManager pStructureManager, Executor pDispatcher, ChunkGenerator p_184014_, int p_184015_, int pSimulationDistance, boolean pSync, ChunkProgressListener pProgressListener, ChunkStatusUpdateListener pStatusListener, Supplier<DimensionDataStorage> pOverworldDataStorage) {
      this.level = pLevel;
      this.mainThreadProcessor = new ServerChunkCache.MainThreadExecutor(pLevel);
      this.mainThread = Thread.currentThread();
      File file1 = p_184010_.getDimensionPath(pLevel.dimension()).resolve("data").toFile();
      file1.mkdirs();
      this.dataStorage = new DimensionDataStorage(file1, pFixerUpper);
      this.chunkMap = new ChunkMap(pLevel, p_184010_, pFixerUpper, pStructureManager, pDispatcher, this.mainThreadProcessor, this, p_184014_, pProgressListener, pStatusListener, pOverworldDataStorage, p_184015_, pSync);
