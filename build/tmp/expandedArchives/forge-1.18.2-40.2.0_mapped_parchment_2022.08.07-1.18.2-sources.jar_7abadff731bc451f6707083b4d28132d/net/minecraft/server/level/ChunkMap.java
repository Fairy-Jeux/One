...
public class ChunkMap extends ChunkStorage implements ChunkHolder.PlayerProvider {
   ...
   public ChunkMap(ServerLevel pLevel, LevelStorageSource.LevelStorageAccess pLevelStorageAccess, DataFixer pFixerUpper, StructureManager pStructureManager, Executor pDispatcher, BlockableEventLoop<Runnable> pMainThreadExecutor, LightChunkGetter p_143046_, ChunkGenerator pGenerator, ChunkProgressListener pProgressListener, ChunkStatusUpdateListener pChunkStatusListener, Supplier<DimensionDataStorage> pOverworldDataStorage, int pViewDistance, boolean pSync) {
      ...
      ProcessorMailbox<Runnable> processormailbox = ProcessorMailbox.create(pDispatcher, "worldgen");
      ...
      ProcessorMailbox<Runnable> processormailbox1 = ProcessorMailbox.create(pDispatcher, "light");
      this.queueSorter = new ChunkTaskPriorityQueueSorter(ImmutableList.of(processormailbox, processorhandle, processormailbox1), pDispatcher, Integer.MAX_VALUE);
      ...
      this.poiManager = new PoiManager(path.resolve("poi"), pFixerUpper, pSync, pLevel);
   ...
   public CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> schedule(ChunkHolder pHolder, ChunkStatus pStatus) {
      ...
      if (pStatus == ChunkStatus.EMPTY) {
         ...
         if (optional.isPresent() && optional.get().getStatus().isOrAfter(pStatus)) {
            CompletableFuture<Either<ChunkAccess, ChunkHolder.ChunkLoadingFailure>> completablefuture = pStatus.load(this.level, this.structureManager, this.lightEngine, (p_203081_) -> {
   ...
   public CompletableFuture<Either<LevelChunk, ChunkHolder.ChunkLoadingFailure>> prepareTickingChunk(ChunkHolder pHolder) {
      ...
      CompletableFuture<Either<LevelChunk, ChunkHolder.ChunkLoadingFailure>> completablefuture1 = completablefuture.thenApplyAsync((p_203067_) -> {
         ...
         this.mainThreadMailbox.tell(ChunkTaskPriorityQueueSorter.message(pHolder, p_203084_));
      ...
      completablefuture1.thenAcceptAsync((p_212860_) -> {
         ...
         this.mainThreadMailbox.tell(ChunkTaskPriorityQueueSorter.message(pHolder, p_212876_));
      });
      return completablefuture1;
   }

   public CompletableFuture<Either<LevelChunk, ChunkHolder.ChunkLoadingFailure>> prepareAccessibleChunk(ChunkHolder pHolder) {
      return this.getChunkRangeFuture(pHolder.getPos(), 1, ChunkStatus::getStatusAroundFullChunk).thenApplyAsync((p_212865_) -> {
         return p_212865_.mapLeft((p_212869_) -> {
            return (LevelChunk)p_212869_.get(p_212869_.size() / 2);
         });
      }, (p_212852_) -> {
         this.mainThreadMailbox.tell(ChunkTaskPriorityQueueSorter.message(pHolder, p_212852_));
      });
   }
   ...
   private boolean isExistingChunkFull(ChunkPos pChunkPos) {
      ...
      if (b0 != 0) {
         ...
         try {
            compoundtag = this.readChunk(pChunkPos);
            if (compoundtag == null) {
               this.markPositionReplaceable(pChunkPos);
               return false;
            }
