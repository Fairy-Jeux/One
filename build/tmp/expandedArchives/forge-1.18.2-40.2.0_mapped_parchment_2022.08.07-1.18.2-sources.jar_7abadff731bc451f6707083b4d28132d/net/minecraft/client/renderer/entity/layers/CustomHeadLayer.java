...
public class CustomHeadLayer<T extends LivingEntity, M extends EntityModel<T> & HeadedModel> extends RenderLayer<T, M> {
   ...
   public CustomHeadLayer(RenderLayerParent<T, M> pRenderer, EntityModelSet pSkullModels) {
      this(pRenderer, pSkullModels, 1.0F, 1.0F, 1.0F);
   }

   public CustomHeadLayer(RenderLayerParent<T, M> pRenderer, EntityModelSet pSkullModels, float pScaleX, float pScaleY, float pScaleZ) {
      super(pRenderer);
      this.scaleX = pScaleX;
      this.scaleY = pScaleY;
      this.scaleZ = pScaleZ;
      this.skullModels = SkullBlockRenderer.createSkullRenderers(pSkullModels);
   }
