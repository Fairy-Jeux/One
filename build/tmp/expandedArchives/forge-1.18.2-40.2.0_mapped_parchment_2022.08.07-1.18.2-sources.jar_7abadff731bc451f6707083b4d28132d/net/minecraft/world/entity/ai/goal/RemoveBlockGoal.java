...
public class RemoveBlockGoal extends MoveToBlockGoal {
   ...
   protected boolean isValidTarget(LevelReader pLevel, BlockPos pPos) {
      ...
      if (chunkaccess == null) {
         ...
         net.minecraft.world.level.block.state.BlockState state = chunkaccess.getBlockState(pPos);
         if (!(state.canEntityDestroy(pLevel, pPos, this.removerMob) && net.minecraftforge.event.ForgeEventFactory.onEntityDestroyBlock(this.removerMob, pPos, state))) return false;
         return state.is(this.blockToRemove) && chunkaccess.getBlockState(pPos.above()).isAir() && chunkaccess.getBlockState(pPos.above(2)).isAir();
      }
   }
}
