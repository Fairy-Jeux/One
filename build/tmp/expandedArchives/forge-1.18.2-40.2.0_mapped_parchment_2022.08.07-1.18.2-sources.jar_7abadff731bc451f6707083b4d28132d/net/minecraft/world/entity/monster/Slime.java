...
public class Slime extends Mob implements Enemy {
   ...
   public static boolean checkSlimeSpawnRules(EntityType<Slime> pSlime, LevelAccessor pLevel, MobSpawnType pSpawnType, BlockPos pPos, Random pRandom) {
      if (pLevel.getDifficulty() != Difficulty.PEACEFUL) {
         if (pLevel.getBiome(pPos).is(Biomes.SWAMP) && pPos.getY() > 50 && pPos.getY() < 70 && pRandom.nextFloat() < 0.5F && pRandom.nextFloat() < pLevel.getMoonBrightness() && pLevel.getMaxLocalRawBrightness(pPos) <= pRandom.nextInt(8)) {
