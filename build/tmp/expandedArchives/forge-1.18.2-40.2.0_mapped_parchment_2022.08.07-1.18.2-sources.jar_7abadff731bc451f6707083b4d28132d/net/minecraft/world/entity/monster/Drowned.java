...
public class Drowned extends Zombie implements RangedAttackMob {
   ...
   public static boolean checkDrownedSpawnRules(EntityType<Drowned> pDrowned, ServerLevelAccessor pServerLevel, MobSpawnType pSpawnType, BlockPos pPos, Random pRandom) {
      if (!pServerLevel.getFluidState(pPos.below()).is(FluidTags.WATER)) {
         return false;
      } else {
         Holder<Biome> holder = pServerLevel.getBiome(pPos);
         boolean flag = pServerLevel.getDifficulty() != Difficulty.PEACEFUL && isDarkEnoughToSpawn(pServerLevel, pPos, pRandom) && (pSpawnType == MobSpawnType.SPAWNER || pServerLevel.getFluidState(pPos).is(FluidTags.WATER));
