...
public class TropicalFish extends AbstractSchoolingFish {
   ...
   public void saveToBucketTag(ItemStack pBucketStack) {
      ...
      CompoundTag compoundtag = pBucketStack.getOrCreateTag();
   ...
   public static boolean checkTropicalFishSpawnRules(EntityType<TropicalFish> pEntityType, LevelAccessor pLevel, MobSpawnType pSpawnType, BlockPos pPos, Random pRandom) {
      return pLevel.getFluidState(pPos.below()).is(FluidTags.WATER) && pLevel.getBlockState(pPos.above()).is(Blocks.WATER) && (pLevel.getBiome(pPos).is(Biomes.LUSH_CAVES) || WaterAnimal.checkSurfaceWaterAnimalSpawnRules(pEntityType, pLevel, pSpawnType, pPos, pRandom));
   }
