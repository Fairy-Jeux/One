...
public class YieldJobSite extends Behavior<Villager> {
   ...
   private boolean nearbyWantsJobsite(PoiType pPoiType, Villager pVillager, BlockPos pPotentialJobSitePos) {
      ...
      if (flag) {
         ...
         if (pVillager.getVillagerData().getProfession() != VillagerProfession.NONE && villagerprofession.getJobPoiType().getPredicate().test(pPoiType)) {
            return !optional.isPresent() ? this.canReachPos(pVillager, pPotentialJobSitePos, pPoiType) : optional.get().pos().equals(pPotentialJobSitePos);
         } else {
            return false;
         }
      }
   ...
   private boolean canReachPos(Villager pVillager, BlockPos pPos, PoiType pPoiType) {
      Path path = pVillager.getNavigation().createPath(pPos, pPoiType.getValidRange());
