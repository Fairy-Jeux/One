...
public abstract class ChunkGenerator implements BiomeManager.NoiseBiomeSource {
   ...
   protected static final <T extends ChunkGenerator> P1<Mu<T>, Registry<StructureSet>> commonCodec(Instance<T> pInstance) {
   ...
   private CompletableFuture<List<ChunkPos>> generateRingPositions(Holder<StructureSet> pSetHolder, ConcentricRingsStructurePlacement pPlacement) {
      return pPlacement.count() == 0 ? CompletableFuture.completedFuture(List.of()) : CompletableFuture.supplyAsync(Util.wrapThreadWithTaskName("placement calculation", () -> {
         ...
         Set<Holder<Biome>> set = pSetHolder.value().structures().stream().flatMap((p_208015_) -> {
         ...
         LOGGER.debug("Calculation for {} took {}s", pSetHolder, d2);
         return list;
   ...
   public CompletableFuture<ChunkAccess> createBiomes(Registry<Biome> pBiomeRegistry, Executor pExecutor, Blender pBlender, StructureFeatureManager pManager, ChunkAccess pAccess) {
      return CompletableFuture.supplyAsync(Util.wrapThreadWithTaskName("init_biomes", () -> {
         pAccess.fillBiomesFromNoise(this.runtimeBiomeSource::getNoiseBiome, this.climateSampler());
         return pAccess;
      }), Util.backgroundExecutor());
   }
   ...
   @Nullable
   private BlockPos getNearestGeneratedStructure(BlockPos pPos, ConcentricRingsStructurePlacement pPlacement) {
      List<ChunkPos> list = this.getRingPositionsFor(pPlacement);
      if (list == null) {
      ...
      for(int j = -pZ; j <= pZ; ++j) {
         ...
         for(int k = -pZ; k <= pZ; ++k) {
            ...
            if (flag || flag1) {
               int l = pX + i * j;
               int i1 = pY + i * k;
               ChunkPos chunkpos = pSpreadPlacement.getPotentialFeatureChunk(pSeed, l, i1);

               for(Holder<ConfiguredStructureFeature<?, ?>> holder : pHolderSet) {
                  StructureCheckResult structurecheckresult = pManager.checkStructurePresence(chunkpos, holder.value(), pSkipKnownStructures);
                  if (structurecheckresult != StructureCheckResult.START_NOT_PRESENT) {
                     if (!pSkipKnownStructures && structurecheckresult == StructureCheckResult.START_PRESENT) {
                        return Pair.of(StructureFeature.getLocatePos(pSpreadPlacement, chunkpos), holder);
                     }

                     ChunkAccess chunkaccess = pReader.getChunk(chunkpos.x, chunkpos.z, ChunkStatus.STRUCTURE_STARTS);
                     StructureStart structurestart = pManager.getStartForFeature(SectionPos.bottomOf(chunkaccess), holder.value(), chunkaccess);
                     if (structurestart != null && structurestart.isValid()) {
                        if (pSkipKnownStructures && structurestart.canBeReferenced()) {
                           pManager.addReference(structurestart);
                           return Pair.of(StructureFeature.getLocatePos(pSpreadPlacement, structurestart.getChunkPos()), holder);
                        }
   ...
   public boolean hasFeatureChunkInRange(ResourceKey<StructureSet> pKey, long pSeed, int pX, int pZ, int pOffset) {
      StructureSet structureset = this.structureSets.get(pKey);
      if (structureset == null) {
         ...
         for(int i = pX - pOffset; i <= pX + pOffset; ++i) {
            for(int j = pZ - pOffset; j <= pZ + pOffset; ++j) {
               if (structureplacement.isFeatureChunk(this, pSeed, i, j)) {
                  return true;
               }
            }
         }
   ...
   public static Stream<ConfiguredStructureFeature<?, ?>> allConfigurations(Registry<ConfiguredStructureFeature<?, ?>> pRegistry, StructureFeature<?> pFeature) {
      return pRegistry.stream().filter((p_211656_) -> {
         return p_211656_.feature == pFeature;
      });
   }
   ...
   private boolean tryGenerateStructure(StructureSet.StructureSelectionEntry pStructureSelectionEntry, StructureFeatureManager pFeatureManager, RegistryAccess pRegistry, StructureManager pStructureManager, long pSeed, ChunkAccess pChunk, ChunkPos pChunkPos, SectionPos pSectionPos) {
      ConfiguredStructureFeature<?, ?> configuredstructurefeature = pStructureSelectionEntry.structure().value();
      int i = fetchReferences(pFeatureManager, pChunk, pSectionPos, configuredstructurefeature);
      ...
      StructureStart structurestart = configuredstructurefeature.generate(pRegistry, this, this.biomeSource, pStructureManager, pSeed, pChunkPos, i, pChunk, predicate);
      if (structurestart.isValid()) {
         pFeatureManager.setStartForFeature(pSectionPos, configuredstructurefeature, structurestart, pChunk);
         return true;
      } else {
         return false;
      }
   ...
   private static int fetchReferences(StructureFeatureManager pFeatureManager, ChunkAccess pChunk, SectionPos pPos, ConfiguredStructureFeature<?, ?> pFeature) {
      StructureStart structurestart = pFeatureManager.getStartForFeature(pPos, pFeature, pChunk);
   ...
   public abstract CompletableFuture<ChunkAccess> fillFromNoise(Executor pExecutor, Blender pBlender, StructureFeatureManager pStructureFeatureManager, ChunkAccess pChunk);
   ...
   private List<StructurePlacement> getPlacementsForFeature(Holder<ConfiguredStructureFeature<?, ?>> pFeatureHolder) {
      this.ensureStructuresGenerated();
      return this.placementsForFeature.getOrDefault(pFeatureHolder.value(), List.of());
   }

   public abstract void addDebugScreenInfo(List<String> pInfo, BlockPos pPos);
