...
public class RootSystemFeature extends Feature<RootSystemConfiguration> {
   ...
   private static void placeDirt(BlockPos pPos, int p_198351_, WorldGenLevel pLevel, RootSystemConfiguration pConfig, Random pRandom) {
      int i = pPos.getX();
      int j = pPos.getZ();
      BlockPos.MutableBlockPos blockpos$mutableblockpos = pPos.mutable();

      for(int k = pPos.getY(); k < p_198351_; ++k) {
         placeRootedDirt(pLevel, pConfig, pRandom, i, j, blockpos$mutableblockpos.set(i, k, j));
      }

   }

   private static void placeRootedDirt(WorldGenLevel pLevel, RootSystemConfiguration pConfig, Random pRandom, int p_160243_, int p_160244_, BlockPos.MutableBlockPos pPos) {
