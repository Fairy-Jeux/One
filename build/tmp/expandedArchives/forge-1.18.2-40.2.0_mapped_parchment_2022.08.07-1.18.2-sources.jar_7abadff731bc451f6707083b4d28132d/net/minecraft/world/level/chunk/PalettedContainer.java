...
public class PalettedContainer<T> implements PaletteResize<T> {
   ...
   public static <T> Codec<PalettedContainer<T>> codec(IdMap<T> pRegistry, Codec<T> pCodec, PalettedContainer.Strategy pStrategy, T pValue) {
      return RecordCodecBuilder.<PalettedContainer.DiscData<T>>create((p_188047_) -> {
         return p_188047_.group(pCodec.mapResult(ExtraCodecs.orElsePartial(pValue)).listOf().fieldOf("palette").forGetter(PalettedContainer.DiscData::paletteEntries), Codec.LONG_STREAM.optionalFieldOf("data").forGetter(PalettedContainer.DiscData::storage)).apply(p_188047_, PalettedContainer.DiscData::new);
      }).comapFlatMap((p_188081_) -> {
         return read(pRegistry, pStrategy, p_188081_);
      }, (p_188074_) -> {
         return p_188074_.write(pRegistry, pStrategy);
      });
   }
   ...
   public void getAll(Consumer<T> pConsumer) {
      ...
      intset.forEach((p_196884_) -> {
         pConsumer.accept(palette.valueFor(p_196884_));
      });
   }
   ...
   private static <T> DataResult<PalettedContainer<T>> read(IdMap<T> pRegistry, PalettedContainer.Strategy pStrategy, PalettedContainer.DiscData<T> pData) {
      List<T> list = pData.paletteEntries();
      ...
      if (j == 0) {
         ...
         Optional<LongStream> optional = pData.storage();
         if (optional.isEmpty()) {
         ...
         try {
            if (configuration.factory() == PalettedContainer.Strategy.GLOBAL_PALETTE_FACTORY) {
               Palette<T> palette = new HashMapPalette<>(pRegistry, j, (p_196886_, p_196887_) -> {
                  return 0;
               }, list);
               ...
               swapPalette(aint, (p_198185_) -> {
                  return pRegistry.getId(palette.valueFor(p_198185_));
               });
   ...
   private PalettedContainer.DiscData<T> write(IdMap<T> pIdMap, PalettedContainer.Strategy pStrategy) {
      this.acquire();

      PalettedContainer.DiscData palettedcontainer$discdata;
      try {
         HashMapPalette<T> hashmappalette = new HashMapPalette<>(pIdMap, this.data.storage.getBits(), this.dummyPaletteResize);
         int i = pStrategy.size();
         int[] aint = new int[i];
         ...
         int j = pStrategy.calculateBitsForSerialization(pIdMap, hashmappalette.getSize());
