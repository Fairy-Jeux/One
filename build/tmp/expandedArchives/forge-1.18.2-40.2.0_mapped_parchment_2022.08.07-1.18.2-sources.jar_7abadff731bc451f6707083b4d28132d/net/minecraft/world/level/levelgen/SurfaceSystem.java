...
public class SurfaceSystem {
   ...
   public SurfaceSystem(Registry<NormalNoise.NoiseParameters> pNoises, BlockState pDefaultBlock, int pSeaLevel, long p_198288_, WorldgenRandom.Algorithm p_198289_) {
      this.noises = pNoises;
      this.defaultBlock = pDefaultBlock;
      this.seaLevel = pSeaLevel;
      this.randomFactory = p_198289_.newInstance(p_198288_).forkPositional();
      this.clayBandsOffsetNoise = Noises.instantiate(pNoises, this.randomFactory, Noises.CLAY_BANDS_OFFSET);
      this.clayBands = generateBands(this.randomFactory.fromHashOf(new ResourceLocation("clay_bands")));
      this.surfaceNoise = Noises.instantiate(pNoises, this.randomFactory, Noises.SURFACE);
      this.surfaceSecondaryNoise = Noises.instantiate(pNoises, this.randomFactory, Noises.SURFACE_SECONDARY);
      this.badlandsPillarNoise = Noises.instantiate(pNoises, this.randomFactory, Noises.BADLANDS_PILLAR);
      this.badlandsPillarRoofNoise = Noises.instantiate(pNoises, this.randomFactory, Noises.BADLANDS_PILLAR_ROOF);
      this.badlandsSurfaceNoise = Noises.instantiate(pNoises, this.randomFactory, Noises.BADLANDS_SURFACE);
      this.icebergPillarNoise = Noises.instantiate(pNoises, this.randomFactory, Noises.ICEBERG_PILLAR);
      this.icebergPillarRoofNoise = Noises.instantiate(pNoises, this.randomFactory, Noises.ICEBERG_PILLAR_ROOF);
      this.icebergSurfaceNoise = Noises.instantiate(pNoises, this.randomFactory, Noises.ICEBERG_SURFACE);
   }

   protected NormalNoise getOrCreateNoise(ResourceKey<NormalNoise.NoiseParameters> pParamaters) {
      return this.noiseIntances.computeIfAbsent(pParamaters, (p_189987_) -> {
         return Noises.instantiate(this.noises, this.randomFactory, pParamaters);
      });
   }

   protected PositionalRandomFactory getOrCreateRandomFactory(ResourceLocation pLocation) {
      return this.positionalRandoms.computeIfAbsent(pLocation, (p_189992_) -> {
         return this.randomFactory.fromHashOf(pLocation).forkPositional();
      });
   }
   ...
   public Optional<BlockState> topMaterial(SurfaceRules.RuleSource pRule, CarvingContext pContext, Function<BlockPos, Holder<Biome>> pBiomeGetter, ChunkAccess pChunk, NoiseChunk pNoiseChunk, BlockPos pPos, boolean p_189978_) {
      SurfaceRules.Context surfacerules$context = new SurfaceRules.Context(this, pChunk, pNoiseChunk, pBiomeGetter, pContext.registryAccess().registryOrThrow(Registry.BIOME_REGISTRY), pContext);
