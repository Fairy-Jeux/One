...
public class StructureCheck {
   ...
   public StructureCheck(ChunkScanAccess pStorageAccess, RegistryAccess pRegistryAccess, StructureManager pStructureManager, ResourceKey<Level> pDimension, ChunkGenerator pChunkGenerator, LevelHeightAccessor pHeightAccessor, BiomeSource pBiomeSource, long pSeed, DataFixer pFixerUpper) {
      ...
      this.structureConfigs = pRegistryAccess.ownedRegistryOrThrow(Registry.CONFIGURED_STRUCTURE_FEATURE_REGISTRY);
   }

   public StructureCheckResult checkStart(ChunkPos pChunkPos, ConfiguredStructureFeature<?, ?> pFeature, boolean pSkipKnownStructures) {
      long i = pChunkPos.toLong();
      Object2IntMap<ConfiguredStructureFeature<?, ?>> object2intmap = this.loadedChunks.get(i);
      if (object2intmap != null) {
         return this.checkStructureInfo(object2intmap, pFeature, pSkipKnownStructures);
      } else {
         StructureCheckResult structurecheckresult = this.tryLoadFromStorage(pChunkPos, pFeature, pSkipKnownStructures, i);
         if (structurecheckresult != null) {
            return structurecheckresult;
         } else {
            boolean flag = this.featureChecks.computeIfAbsent(pFeature, (p_209974_) -> {
               return new Long2BooleanOpenHashMap();
            }).computeIfAbsent(i, (p_209963_) -> {
               return this.canCreateStructure(pChunkPos, pFeature);
            });
   ...
   @Nullable
   private Object2IntMap<ConfiguredStructureFeature<?, ?>> loadStructures(CompoundTag pTag) {
