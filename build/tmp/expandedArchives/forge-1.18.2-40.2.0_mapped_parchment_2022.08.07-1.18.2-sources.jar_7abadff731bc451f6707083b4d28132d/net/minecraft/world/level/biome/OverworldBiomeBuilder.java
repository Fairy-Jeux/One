...
public final class OverworldBiomeBuilder {
   ...
   protected void addBiomes(Consumer<Pair<Climate.ParameterPoint, ResourceKey<Biome>>> pKey) {
      if (SharedConstants.debugGenerateSquareTerrainWithoutNoise) {
         TerrainProvider.overworld(false).addDebugBiomesToVisualizeSplinePoints(pKey);
      } else {
   ...
   private void addMidSlice(Consumer<Pair<Climate.ParameterPoint, ResourceKey<Biome>>> pConsumer, Climate.Parameter pParam) {
      ...
      this.addSurfaceBiome(pConsumer, this.UNFROZEN_RANGE, this.FULL_RANGE, Climate.Parameter.span(this.nearInlandContinentalness, this.farInlandContinentalness), this.erosions[6], pParam, 0.0F, Biomes.SWAMP);
   ...
   private void addValleys(Consumer<Pair<Climate.ParameterPoint, ResourceKey<Biome>>> pConsumer, Climate.Parameter pParam) {
      ...
      this.addSurfaceBiome(pConsumer, this.UNFROZEN_RANGE, this.FULL_RANGE, Climate.Parameter.span(this.inlandContinentalness, this.farInlandContinentalness), this.erosions[6], pParam, 0.0F, Biomes.SWAMP);
   ...
   private ResourceKey<Biome> pickBadlandsBiome(int pHumidity, Climate.Parameter pParam) {
      if (pHumidity < 2) {
         return pParam.max() < 0L ? Biomes.ERODED_BADLANDS : Biomes.BADLANDS;
      } else {
   ...
   public static String getDebugStringForPeaksAndValleys(double pPeaksAndValleysData) {
      if (pPeaksAndValleysData < (double)TerrainShaper.peaksAndValleys(0.05F)) {
         return "Valley";
      } else if (pPeaksAndValleysData < (double)TerrainShaper.peaksAndValleys(0.26666668F)) {
         return "Low";
      } else if (pPeaksAndValleysData < (double)TerrainShaper.peaksAndValleys(0.4F)) {
         return "Mid";
      } else {
         return pPeaksAndValleysData < (double)TerrainShaper.peaksAndValleys(0.56666666F) ? "High" : "Peak";
      }
   }
