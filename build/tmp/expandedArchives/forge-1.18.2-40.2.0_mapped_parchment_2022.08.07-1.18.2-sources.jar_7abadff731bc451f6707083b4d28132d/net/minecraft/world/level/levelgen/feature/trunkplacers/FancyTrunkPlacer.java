...
public class FancyTrunkPlacer extends TrunkPlacer {
   ...
   private boolean makeLimb(LevelSimulatedReader pLevel, BiConsumer<BlockPos, BlockState> pBlockSetter, Random pRandom, BlockPos pBasePos, BlockPos pOffsetPos, boolean p_161821_, TreeConfiguration pConfig) {
      if (!p_161821_ && Objects.equals(pBasePos, pOffsetPos)) {
         return true;
      } else {
         ...
         for(int j = 0; j <= i; ++j) {
            ...
            if (p_161821_) {
               TrunkPlacer.placeLog(pLevel, pBlockSetter, pRandom, blockpos1, pConfig, (p_161826_) -> {
                  return p_161826_.setValue(RotatedPillarBlock.AXIS, this.getLogAxis(pBasePos, blockpos1));
               });
            } else if (!TreeFeature.isFree(pLevel, blockpos1)) {
               return false;
            }
         }
   ...
   private void makeBranches(LevelSimulatedReader pLevel, BiConsumer<BlockPos, BlockState> p_161809_, Random pRandom, int p_161811_, BlockPos pPos, List<FancyTrunkPlacer.FoliageCoords> pFoliageCoords, TreeConfiguration pConfig) {
      for(FancyTrunkPlacer.FoliageCoords fancytrunkplacer$foliagecoords : pFoliageCoords) {
         ...
         if (!blockpos.equals(fancytrunkplacer$foliagecoords.attachment.pos()) && this.trimBranches(p_161811_, i - pPos.getY())) {
            this.makeLimb(pLevel, p_161809_, pRandom, blockpos, fancytrunkplacer$foliagecoords.attachment.pos(), true, pConfig);
         }
      }
