...
public class NoiseRouterData {
   ...
   protected static NoiseRouterWithOnlyNoises overworld(NoiseSettings pNoiseSettings, boolean pLargeBiomes) {
      return overworldWithNewCaves(pNoiseSettings, pLargeBiomes);
   }
   ...
   private static DensityFunction underground(DensityFunction pDensityFunction) {
      ...
      DensityFunction densityfunction5 = DensityFunctions.add(DensityFunctions.add(DensityFunctions.constant(0.27D), densityfunction4).clamp(-1.0D, 1.0D), DensityFunctions.add(DensityFunctions.constant(1.5D), DensityFunctions.mul(DensityFunctions.constant(-0.64D), pDensityFunction)).clamp(0.0D, 0.5D));
   ...
   private static DensityFunction postProcess(NoiseSettings pNoiseSettings, DensityFunction pDensityFunction) {
      DensityFunction densityfunction = DensityFunctions.slide(pNoiseSettings, pDensityFunction);
   ...
   private static NoiseRouterWithOnlyNoises overworldWithNewCaves(NoiseSettings pNoiseSettings, boolean pLargeBiomes) {
      ...
      DensityFunction densityfunction6 = DensityFunctions.shiftedNoise2d(densityfunction4, densityfunction5, 0.25D, getNoise(pLargeBiomes ? Noises.TEMPERATURE_LARGE : Noises.TEMPERATURE));
      DensityFunction densityfunction7 = DensityFunctions.shiftedNoise2d(densityfunction4, densityfunction5, 0.25D, getNoise(pLargeBiomes ? Noises.VEGETATION_LARGE : Noises.VEGETATION));
      DensityFunction densityfunction8 = getFunction(pLargeBiomes ? FACTOR_LARGE : FACTOR);
      DensityFunction densityfunction9 = getFunction(pLargeBiomes ? DEPTH_LARGE : DEPTH);
      DensityFunction densityfunction10 = noiseGradientDensity(DensityFunctions.cache2d(densityfunction8), densityfunction9);
      DensityFunction densityfunction11 = getFunction(pLargeBiomes ? SLOPED_CHEESE_LARGE : SLOPED_CHEESE);
      ...
      DensityFunction densityfunction14 = DensityFunctions.min(postProcess(pNoiseSettings, densityfunction13), getFunction(NOODLE));
      DensityFunction densityfunction15 = getFunction(Y);
      int i = pNoiseSettings.minY();
      ...
      return new NoiseRouterWithOnlyNoises(densityfunction, densityfunction1, densityfunction2, densityfunction3, densityfunction6, densityfunction7, getFunction(pLargeBiomes ? CONTINENTS_LARGE : CONTINENTS), getFunction(pLargeBiomes ? EROSION_LARGE : EROSION), getFunction(pLargeBiomes ? DEPTH_LARGE : DEPTH), getFunction(RIDGES), densityfunction10, densityfunction14, densityfunction16, densityfunction19, densityfunction20);
   }

   private static NoiseRouterWithOnlyNoises noNewCaves(NoiseSettings pNoiseSettings) {
      ...
      DensityFunction densityfunction5 = postProcess(pNoiseSettings, getFunction(SLOPED_CHEESE));
   ...
   protected static NoiseRouterWithOnlyNoises overworldWithoutCaves(NoiseSettings pNoiseSettings) {
      return noNewCaves(pNoiseSettings);
   }

   protected static NoiseRouterWithOnlyNoises nether(NoiseSettings pNoiseSettings) {
      return noNewCaves(pNoiseSettings);
   }

   protected static NoiseRouterWithOnlyNoises end(NoiseSettings pNoiseSettings) {
      DensityFunction densityfunction = DensityFunctions.cache2d(DensityFunctions.endIslands(0L));
      DensityFunction densityfunction1 = postProcess(pNoiseSettings, getFunction(SLOPED_CHEESE_END));
   ...
   public static NoiseRouter createNoiseRouter(NoiseSettings pNoiseSettings, long p_209504_, Registry<NormalNoise.NoiseParameters> p_209505_, WorldgenRandom.Algorithm p_209506_, NoiseRouterWithOnlyNoises p_209507_) {
      ...
      DensityFunction.Visitor densityfunction$visitor = (p_209535_) -> {
         if (p_209535_ instanceof DensityFunctions.Noise) {
            ...
            return flag ? new BlendedNoise(p_209506_.newInstance(p_209504_), pNoiseSettings.noiseSamplingSettings(), pNoiseSettings.getCellWidth(), pNoiseSettings.getCellHeight()) : new BlendedNoise(positionalrandomfactory.fromHashOf(new ResourceLocation("terrain")), pNoiseSettings.noiseSamplingSettings(), pNoiseSettings.getCellWidth(), pNoiseSettings.getCellHeight());
            ...
            TerrainShaper terrainshaper = pNoiseSettings.terrainShaper();
            ...
            return new DensityFunctions.Slide(pNoiseSettings, densityfunctions$slide.input());
         } else {
            return p_209535_;
         }
      };
   ...
   protected static double applySlide(NoiseSettings pNoiseSettings, double p_209500_, double p_209501_) {
      double d0 = (double)((int)p_209501_ / pNoiseSettings.getCellHeight() - pNoiseSettings.getMinCellY());
      p_209500_ = pNoiseSettings.topSlideSettings().applySlide(p_209500_, (double)pNoiseSettings.getCellCountY() - d0);
      return pNoiseSettings.bottomSlideSettings().applySlide(p_209500_, d0);
   }
