...
public record TerrainShaper(CubicSpline<TerrainShaper.Point> offsetSampler, CubicSpline<TerrainShaper.Point> factorSampler, CubicSpline<TerrainShaper.Point> jaggednessSampler) {
   ...
   public static TerrainShaper overworld(boolean pTransform) {
      ToFloatFunction<Float> tofloatfunction = pTransform ? TerrainShaper::getAmplifiedOffset : NO_TRANSFORM;
      ToFloatFunction<Float> tofloatfunction1 = pTransform ? TerrainShaper::getAmplifiedFactor : NO_TRANSFORM;
      ToFloatFunction<Float> tofloatfunction2 = pTransform ? TerrainShaper::getAmplifiedJaggedness : NO_TRANSFORM;
   ...
   private static CubicSpline<TerrainShaper.Point> buildErosionJaggednessSpline(float pX, float pY, float pDeltaX, float pDeltaY, ToFloatFunction<Float> pOutputMapper) {
      float f = -0.5775F;
      CubicSpline<TerrainShaper.Point> cubicspline = buildRidgeJaggednessSpline(pX, pDeltaX, pOutputMapper);
      CubicSpline<TerrainShaper.Point> cubicspline1 = buildRidgeJaggednessSpline(pY, pDeltaY, pOutputMapper);
      return CubicSpline.builder(TerrainShaper.Coordinate.EROSION, pOutputMapper).addPoint(-1.0F, cubicspline, 0.0F).addPoint(-0.78F, cubicspline1, 0.0F).addPoint(-0.5775F, cubicspline1, 0.0F).addPoint(-0.375F, 0.0F, 0.0F).build();
   }

   private static CubicSpline<TerrainShaper.Point> buildRidgeJaggednessSpline(float pPoint, float pDelta, ToFloatFunction<Float> pOutputMapper) {
      float f = peaksAndValleys(0.4F);
      float f1 = peaksAndValleys(0.56666666F);
      float f2 = (f + f1) / 2.0F;
      CubicSpline.Builder<TerrainShaper.Point> builder = CubicSpline.builder(TerrainShaper.Coordinate.RIDGES, pOutputMapper);
      builder.addPoint(f, 0.0F, 0.0F);
      if (pDelta > 0.0F) {
         builder.addPoint(f2, buildWeirdnessJaggednessSpline(pDelta, pOutputMapper), 0.0F);
      } else {
         builder.addPoint(f2, 0.0F, 0.0F);
      }

      if (pPoint > 0.0F) {
         builder.addPoint(1.0F, buildWeirdnessJaggednessSpline(pPoint, pOutputMapper), 0.0F);
      } else {
   ...
   private static CubicSpline<TerrainShaper.Point> buildWeirdnessJaggednessSpline(float pDelta, ToFloatFunction<Float> pOutputMapper) {
      float f = 0.63F * pDelta;
      float f1 = 0.3F * pDelta;
      return CubicSpline.builder(TerrainShaper.Coordinate.WEIRDNESS, pOutputMapper).addPoint(-0.01F, f, 0.0F).addPoint(0.01F, f1, 0.0F).build();
   }

   private static CubicSpline<TerrainShaper.Point> getErosionFactor(float pY, boolean p_187309_, ToFloatFunction<Float> pOutputMapper) {
      CubicSpline<TerrainShaper.Point> cubicspline = CubicSpline.builder(TerrainShaper.Coordinate.WEIRDNESS, pOutputMapper).addPoint(-0.2F, 6.3F, 0.0F).addPoint(0.2F, pY, 0.0F).build();
      CubicSpline.Builder<TerrainShaper.Point> builder = CubicSpline.builder(TerrainShaper.Coordinate.EROSION, pOutputMapper).addPoint(-0.6F, cubicspline, 0.0F).addPoint(-0.5F, CubicSpline.builder(TerrainShaper.Coordinate.WEIRDNESS, pOutputMapper).addPoint(-0.05F, 6.3F, 0.0F).addPoint(0.05F, 2.67F, 0.0F).build(), 0.0F).addPoint(-0.35F, cubicspline, 0.0F).addPoint(-0.25F, cubicspline, 0.0F).addPoint(-0.1F, CubicSpline.builder(TerrainShaper.Coordinate.WEIRDNESS, pOutputMapper).addPoint(-0.05F, 2.67F, 0.0F).addPoint(0.05F, 6.3F, 0.0F).build(), 0.0F).addPoint(0.03F, cubicspline, 0.0F);
      if (p_187309_) {
         CubicSpline<TerrainShaper.Point> cubicspline1 = CubicSpline.builder(TerrainShaper.Coordinate.WEIRDNESS, pOutputMapper).addPoint(0.0F, pY, 0.0F).addPoint(0.1F, 0.625F, 0.0F).build();
         CubicSpline<TerrainShaper.Point> cubicspline2 = CubicSpline.builder(TerrainShaper.Coordinate.RIDGES, pOutputMapper).addPoint(-0.9F, pY, 0.0F).addPoint(-0.69F, cubicspline1, 0.0F).build();
         builder.addPoint(0.35F, pY, 0.0F).addPoint(0.45F, cubicspline2, 0.0F).addPoint(0.55F, cubicspline2, 0.0F).addPoint(0.62F, pY, 0.0F);
      } else {
         CubicSpline<TerrainShaper.Point> cubicspline3 = CubicSpline.builder(TerrainShaper.Coordinate.RIDGES, pOutputMapper).addPoint(-0.7F, cubicspline, 0.0F).addPoint(-0.15F, 1.37F, 0.0F).build();
         CubicSpline<TerrainShaper.Point> cubicspline4 = CubicSpline.builder(TerrainShaper.Coordinate.RIDGES, pOutputMapper).addPoint(0.45F, cubicspline, 0.0F).addPoint(0.7F, 1.56F, 0.0F).build();
         builder.addPoint(0.05F, cubicspline4, 0.0F).addPoint(0.4F, cubicspline4, 0.0F).addPoint(0.45F, cubicspline3, 0.0F).addPoint(0.55F, cubicspline3, 0.0F).addPoint(0.58F, pY, 0.0F);
      }
   ...
   private static CubicSpline<TerrainShaper.Point> buildMountainRidgeSplineWithPoints(float pDelta, boolean p_187332_, ToFloatFunction<Float> pOutputMapper) {
      CubicSpline.Builder<TerrainShaper.Point> builder = CubicSpline.builder(TerrainShaper.Coordinate.RIDGES, pOutputMapper);
      float f = -0.7F;
      float f1 = -1.0F;
      float f2 = mountainContinentalness(-1.0F, pDelta, -0.7F);
      float f3 = 1.0F;
      float f4 = mountainContinentalness(1.0F, pDelta, -0.7F);
      float f5 = calculateMountainRidgeZeroContinentalnessPoint(pDelta);
      float f6 = -0.65F;
      if (-0.65F < f5 && f5 < 1.0F) {
         float f14 = mountainContinentalness(-0.65F, pDelta, -0.7F);
         float f8 = -0.75F;
         float f9 = mountainContinentalness(-0.75F, pDelta, -0.7F);
         ...
         float f11 = mountainContinentalness(f5, pDelta, -0.7F);
   ...
   public float offset(TerrainShaper.Point pPoint) {
      return this.offsetSampler.apply(pPoint) + -0.50375F;
   }

   public float factor(TerrainShaper.Point pPoint) {
      return this.factorSampler.apply(pPoint);
   }

   public float jaggedness(TerrainShaper.Point pPoint) {
      return this.jaggednessSampler.apply(pPoint);
   }

   public static TerrainShaper.Point makePoint(float pContinents, float pErosion, float pWeirdness) {
      return new TerrainShaper.Point(pContinents, pErosion, peaksAndValleys(pWeirdness), pWeirdness);
   }

   public static TerrainShaper.PointCustom makePoint(DensityFunction.FunctionContext pContext) {
      return new TerrainShaper.PointCustom(pContext);
   }
   ...
   protected static enum Coordinate implements StringRepresentable, ToFloatFunction<TerrainShaper.Point> {
      ...
      private Coordinate(ToFloatFunction<TerrainShaper.Point> pReference, String pName) {
