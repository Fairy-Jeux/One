...
public abstract class BiomeSource implements BiomeResolver {
   ...
   private List<BiomeSource.StepFeatureData> buildFeaturesPerStep(List<Holder<Biome>> pPossibleBiomes, boolean pVerify) {
      ...
      for(Holder<Biome> holder : pPossibleBiomes) {
      ...
      for(FeatureData biomesource$1featuredata : map.keySet()) {
         ...
         if (!set.contains(biomesource$1featuredata) && Graph.depthFirstSearch(map, set, set1, list2::add, biomesource$1featuredata)) {
            ...
            List<Holder<Biome>> list3 = new ArrayList<>(pPossibleBiomes);
   ...
   @Nullable
   public Pair<BlockPos, Holder<Biome>> findBiomeHorizontal(int pX, int pY, int pZ, int pRadius, Predicate<Holder<Biome>> pBiomePredicate, Random pRandom, Climate.Sampler pSampler) {
