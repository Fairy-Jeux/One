...
public abstract class Registry<T> implements Keyable, IdMap<T> {
   ...
   private static <T> DefaultedRegistry<T> registerDefaulted(ResourceKey<? extends Registry<T>> pRegistryKey, String pDefaultValueName, Function<T, Holder.Reference<T>> pCustomHolderProvider, Registry.RegistryBootstrap<T> p_206035_) {
      return registerDefaulted(pRegistryKey, pDefaultValueName, Lifecycle.experimental(), pCustomHolderProvider, p_206035_);
   }
   ...
   private static <T> Registry<T> registerSimple(ResourceKey<? extends Registry<T>> pKey, Lifecycle pLifecycle, Function<T, Holder.Reference<T>> pCustomHolderProvider, Registry.RegistryBootstrap<T> p_206007_) {
      return internalRegister(pKey, new MappedRegistry<>(pKey, pLifecycle, pCustomHolderProvider), p_206007_, pLifecycle);
   }
   ...
   private static <T> DefaultedRegistry<T> registerDefaulted(ResourceKey<? extends Registry<T>> pKey, String pDefaultName, Lifecycle pElementsLifecycle, Function<T, Holder.Reference<T>> pCustomHolderProvider, Registry.RegistryBootstrap<T> p_206026_) {
      return internalRegister(pKey, new DefaultedRegistry<>(pDefaultName, pKey, pElementsLifecycle, pCustomHolderProvider), p_206026_, pElementsLifecycle);
   }
   ...
   public abstract Holder<T> getOrCreateHolder(ResourceKey<T> pKey);
